object frmSelectRestore: TfrmSelectRestore
  Left = 0
  Top = 0
  BorderIcons = [biSystemMenu]
  Caption = 'Download to...'
  ClientHeight = 177
  ClientWidth = 439
  Color = clBtnFace
  Constraints.MaxHeight = 215
  Constraints.MaxWidth = 455
  Constraints.MinHeight = 215
  Constraints.MinWidth = 455
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  Icon.Data = {
    }
  OldCreateOrder = False
  Position = poMainFormCenter
  OnClose = FormClose
  OnCreate = FormCreate
  OnDestroy = FormDestroy
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Label1: TLabel
    Left = 76
    Top = 21
    Width = 68
    Height = 13
    Caption = 'Remote Files: '
  end
  object Label2: TLabel
    Left = 76
    Top = 45
    Width = 39
    Height = 13
    Caption = 'Owner: '
  end
  object Label3: TLabel
    Left = 9
    Top = 79
    Width = 67
    Height = 13
    Caption = 'Download to: '
  end
  object lbOwner: TLabel
    Left = 147
    Top = 45
    Width = 40
    Height = 13
    Caption = 'lbOwner'
  end
  object Label4: TLabel
    Left = 9
    Top = 109
    Width = 53
    Height = 13
    Caption = 'File Name: '
  end
  object igShow: TImage
    Left = 21
    Top = 21
    Width = 32
    Height = 32
  end
  object tbtnDownCompleted: TRzToolButton
    Left = 366
    Top = 103
    Width = 64
    Hint = 'download file'
    DropDownMenu = pmDownCompleted
    ShowCaption = True
    UseToolbarShowCaption = False
    ToolStyle = tsDropDown
    Caption = 'Download'
    ParentShowHint = False
    ShowHint = True
    Visible = False
  end
  object btnBrows: TButton
    Left = 366
    Top = 74
    Width = 64
    Height = 25
    Caption = 'Browse...'
    TabOrder = 1
    OnClick = btnBrowsClick
  end
  object edtSharePath: TEdit
    Left = 147
    Top = 21
    Width = 224
    Height = 17
    BorderStyle = bsNone
    Color = clBtnFace
    ImeName = #20013#25991' - QQ'#25340#38899#36755#20837#27861
    ReadOnly = True
    TabOrder = 2
    Text = 'edtSharePath'
  end
  object btnOK: TButton
    Left = 178
    Top = 138
    Width = 75
    Height = 25
    Caption = 'OK'
    TabOrder = 0
    OnClick = btnOKClick
  end
  object btnCancel: TButton
    Left = 283
    Top = 138
    Width = 75
    Height = 25
    Cancel = True
    Caption = 'Cancel'
    TabOrder = 3
    OnClick = btnCancelClick
  end
  object cbbSavePath: TComboBox
    Left = 76
    Top = 76
    Width = 284
    Height = 21
    ImeName = #20013#25991' - QQ'#25340#38899#36755#20837#27861
    TabOrder = 4
    OnSelect = cbbSavePathSelect
  end
  object edtSaveName: TEdit
    Left = 76
    Top = 106
    Width = 284
    Height = 21
    ImeName = #20013#25991' - QQ'#25340#38899#36755#20837#27861
    TabOrder = 5
  end
  object btnPreview: TButton
    Left = 73
    Top = 138
    Width = 75
    Height = 25
    Caption = 'Preview'
    TabOrder = 6
    OnClick = btnPreviewClick
  end
  object pmDownCompleted: TPopupMenu
    Left = 384
    Top = 16
    object miDown: TMenuItem
      AutoCheck = True
      Caption = 'Download File'
      Checked = True
      Hint = 'Download file'
      RadioItem = True
      OnClick = miDownClick
    end
    object miRun: TMenuItem
      AutoCheck = True
      Caption = 'Download and Run File'
      Hint = 'Automatically run on successful download'
      RadioItem = True
      OnClick = miDownClick
    end
    object miExplorer: TMenuItem
      AutoCheck = True
      Caption = 'Download and Explorer File'
      Hint = 'Automatically explorer on successful download'
      RadioItem = True
      OnClick = miDownClick
    end
  end
end
