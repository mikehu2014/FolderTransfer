...
procedure TfrmFreeTips.lkBackupCowLinkClick(Sender: TObject; const Link: string;
  LinkType: TSysLinkType);
begin
  MyInternetExplorer.OpenWeb( MyOtherWebUrl.getBackupCow );
  MarkAdsClick( AdsName_BackupCow );
end;
...
var
  PageShowIndex : Integer;
begin
  Randomize;
  PageShowIndex := Random( ( pcMain.PageCount - 1 ) * 2 );
  if PageShowIndex >= pcMain.PageCount then
    PageShowIndex := 0;
  pcMain.ActivePageIndex := PageShowIndex;
