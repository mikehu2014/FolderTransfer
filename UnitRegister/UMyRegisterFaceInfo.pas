...
uses UChangeInfo, Comctrls, vcl.dialogs, forms, stdctrls, Controls, virtualtrees;
...
{$Region ' 版本限制 数据修改 ' }
  ...
  protected
    ...
    // 访问网站
  TWebsiteAccessFace = class( TFaceChangeInfo )
  private
    WebUrl : string;
  public
    procedure SetWebUrl( _WebUrl : string );
  protected
    procedure Update;override;
  end;
...
uses UFormRegister, UFormFreeEdition, UMainForm, UFormSetting, UMyUtil;
...
begin
  ...
  begin
    try
      ...
      CutWidth := ( 180 - btnCancel.Width ) div 2;
      btnYes.Caption := 'OK';
      btnYes.Left := btnYes.Left - CutWidth;
      btnCancel.Caption := 'Show Edition Comparison'; // 控制按钮的显示文字
      btnCancel.Left := btnCancel.Left - CutWidth;
      btnCancel.Width := 180;
      IsShowEdition := ShowModal = mrCancel;
    except
    end;
    Free;
  end;
    ...
    // 显示 版本比较
  if IsShowEdition then
    frmFreeEdition.ShowModal;
...
procedure TWebsiteAccessFace.SetWebUrl(_WebUrl: string);
begin
  WebUrl := _WebUrl;
end;

procedure TWebsiteAccessFace.Update;
begin
  Application.ProcessMessages;
  MyInternetExplorer.OpenWeb( WebUrl );
end;
