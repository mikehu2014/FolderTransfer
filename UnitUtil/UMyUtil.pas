...
uses SysUtils, Masks, StrUtils, Windows, WinSock, Nb30, Sockets, DateUtils, Math, Controls, ShellAPI,
     ...
     FileCtrl, IniFiles, ComCtrls, ComObj, Vcl.Graphics, jpeg, GIFImg, pngimage, WordXp, zip, VersionInfo,
     mp3_id3v1, TWmaTag, RAR, Generics.Collections, idhttp, PsAPI;

var
  AppInfo_IsDebug : Boolean = False;
...
type
  ...
  public
    ...
    class procedure DeleteFile( FilePath : string );
  end;
  ...
  public
    ...
    class function getIsFAT32( Path : string ): Boolean;  // 是否 FAT32 的磁盘
  public
  ...
  public
    class procedure CloseFolder( FolderPath : string );
  end;
  ...
  public
    ...
    class function getListCount( s, SplitStr: string; MaxCount : Integer ): TStringList;
    class function getListSkip( s, SplitStr, SkipStr: string ): TStringList;
  ...
  MyOSUtil = class
  public
    class function getOsStr : string;
  end;

  ScheduleUtil = class
  public
    class function getMinValue( ItemIndex : Integer ): Integer;
    class function getHourValue( ItemIndex : Integer ): Integer;
  public
    class function getShowWeek( ItemIndex : Integer ): string;
  public
    class function getDateHour( dateValue : TDateTime; hour : Integer ): TDateTime;
    class function getNextBackupTime( ScheduleType, ScheduleValue1, ScheduleValue2 : Integer;
                                      LastSyncTime : TDateTime ): TDateTime;
  end;
...
const
  ScheduleType_Manual = 0;
  ScheduleType_Min = 1;
  ScheduleType_Hour = 2;
  ScheduleType_Day = 3;
  ScheduleType_Week = 4;
  ScheduleType_Month = 5;

const
  DesName_Show = '( %s )';
  DesMyComputerName_Show = 'My Computer ( %s )';
...
var
  XmlConfirm_ThisRun : Boolean = False;  // 是否再次询问管理员权限
...
class procedure MyFileInfo.DeleteFile(FilePath: string);
begin
  try
...
begin
  ...
  Result := ( ExtName = 'zip' ) or ( ExtName = '7z' ) or ( ExtName = 'rar' );
end;
...
class function MyHardDisk.getIsFAT32(Path: string): Boolean;
var
  Driver : string;
...
begin
  try
    Driver := MyFilePath.getPath( ExtractFileDrive( Path ) );
    GetVolumeInformation( Pwidechar( Driver ),VolumeName,256,@VolumeSerialNumber,size1,size2,FileSystemName,256);
    Result := UpperCase( FileSystemName ) = 'FAT32';
  except
    Result := False;
  end;
end;
...
class procedure MyExplore.CloseFolder(FolderPath: string);
var
  FolderName, AppPath : string;
  h:HWND;
  pid: Cardinal;
  pHandle: THandle;
  buf: array[0..MAX_PATH] of Char;
begin
  try
    FolderName := MyFileInfo.getFileName( FolderPath );

    h:=FindWindow( nil, pchar( FolderName ) );; //获取当前激活的窗体句柄
    if h <> 0 then
    begin
      ...
      AppPath := buf;
      if LowerCase( ExtractFileName( AppPath ) ) = 'explorer.exe' then
...
begin
  if FileExists( FilePath ) then
  begin
    CloseFolder( ExtractFileDir( FilePath ) );
    ShellExecute( 0, 'open', 'explorer.exe', PChar('/select,' + FilePath ), nil, SW_SHOW );
  end
  else
  if DirectoryExists( FilePath ) then
    ShellExecute( 0, 'open', 'explorer.exe', PChar( FilePath ), nil, SW_SHOW )
end;
...
begin
  Result := MyAppDataUtil.get + 'MyConfig.dat';
end;
...
class function MySplitStr.getListCount(s, SplitStr: string;
  MaxCount: Integer): TStringList;
var
  FindStr : string;
  BeforePos, AfterPos : Integer;
  LenSplit : Integer;
begin
  ...
  begin
    ...
    if FindStr <> '' then
      Result.Add( FindStr );
    s := Copy( s, Length( FindStr ) + 1, Length( s ) - Length( FindStr ) );
    if ( Result.Count + 1 ) >= MaxCount then
    begin
      Result.Add( s );
      Break;
    end;
...
class function MySplitStr.getListSkip(s, SplitStr,
  SkipStr: string): TStringList;
var
  FindStr : string;
  BeforePos, AfterPos, SkipPos : Integer;
  LenSplit, LenSkip : Integer;
begin
  s := SplitStr + s + SplitStr;
  LenSplit := Length( SplitStr );
  LenSkip := Length( SkipStr );
  ...
  begin
    BeforePos := Pos( SplitStr, s );
    s := Copy( s, LenSplit + 1, Length( s ) - LenSplit );
    if LeftStr( s, LenSkip ) = SkipStr then
    begin
      s := Copy( s, LenSkip + 1, Length( s ) - LenSkip );
      AfterPos := Pos( SkipStr, s );
    end
    else
      AfterPos := Pos( SplitStr, s );
    FindStr := Copy( s, 1, AfterPos - 1 );
...
class function MyAppDataUtil.get: string;
var
  FilePath: array [0..255] of char;
begin
  try     // 系统 Api
...
begin
  ...
  if SplitList.Count >= 4 then
    Result := IntToStr( 25150 + StrToInt( SplitList[3] ) );
...
begin
  try
    ...
    PcIpList.Free;
      ...
      // 不在局域网 Ip 范围
    if ( PcIpFirst <> '10' ) and ( PcIpFirst <> '172' ) and ( PcIpFirst <> '192' ) then
    begin
      Result := False;
      Exit;
    end;
...
begin
  ...
  repeat
    ...
    if Result.IndexOf( BroadcastIp ) < 0  then // 不存在则添加
...
begin
  Result := MyFilePath.getPath( MyAppDataUtil.get ) + 'unrar.dll';
end;
...
begin
  ...
  Result := StrToFloatDef( TimeStr, IncDay( Now, 1 ) );
end;
...
class function MyZipUtil.getZipHeader(FileName, FilePath: string;
  Compression: TZipCompression): TZipHeader;
var
  LHeader: TZipHeader;
begin
  // Setup Header
...
class function MyOSUtil.getOsStr: string;
var
  s : string;
  osi : TOSVersionInfo;
  WindowsVersion:Extended;
begin
  WindowsVersion:=StrToFloat(Format('%d.%d',[Win32MajorVersion,Win32MinorVersion]));
  case Win32Platform of
    Ver_Platform_Win32s: s:='Win3.1';
    Ver_Platform_Win32_Windows:
    if WindowsVersion=4.0 then
      s:='Win95'
    else
    if WindowsVersion=4.9
    then
      s:='WinMe'
    else
      s:='Win98';
    Ver_Platform_Win32_NT:
    if WindowsVersion=3.51 then
      s:='WinNT3.51'
    else
    if WindowsVersion=4.0 then
      s:='WinNT4.0'
    else
    if WindowsVersion=5.0 then
      s:='Win2000'
    else
    ...
    if WindowsVersion=6.2 then
      s:='Win8'
    else
      s:= FloatToStr(WindowsVersion);
    Ver_Platform_Win32_CE: s:='WinCE';
  end;
  Result := s;
end;
...
class function ScheduleUtil.getDateHour(dateValue: TDateTime; hour: Integer): TDateTime;
var
  y, m, d : word;
begin
  DecodeDate( dateValue, y, m, d );
  Result := EncodeDateTime( y, m, d, hour, 0, 0, 0 );
end;

class function ScheduleUtil.getHourValue(ItemIndex: Integer): Integer;
begin
  if ItemIndex < 6 then
    Result := ItemIndex + 1
  else
...
class function ScheduleUtil.getMinValue(ItemIndex: Integer): Integer;
begin
  if ItemIndex = 0 then
    Result := 1
  else
...
class function ScheduleUtil.getNextBackupTime(ScheduleType, ScheduleValue1,
  ScheduleValue2: Integer; LastSyncTime : TDateTime): TDateTime;
var
  ShowTime, ScheduleTime : TDateTime;
  HourInt, DayInt : Integer;
begin
  ...
  begin
    if ScheduleType = ScheduleType_Min then
      ShowTime := IncMinute( LastSyncTime, ScheduleUtil.getMinValue( ScheduleValue1 ) )
    else
    if ScheduleType = ScheduleType_Hour then
      ShowTime := IncHour( LastSyncTime, ScheduleUtil.getHourValue( ScheduleValue1 ) )
    else
    if ScheduleType = ScheduleType_Day then
    begin
      HourInt := ScheduleValue1;
      ScheduleTime := ScheduleUtil.getDateHour( Now, HourInt );
      if LastSyncTime < ScheduleTime then
        ShowTime := ScheduleTime
      else
        ShowTime := IncDay( ScheduleTime, 1 );
    end
    else
    if ScheduleType = ScheduleType_Week then
    begin
      DayInt := ScheduleValue1;
      if DayInt = 0 then
        DayInt := 7;
      DayInt := DayInt - 1;
      HourInt := ScheduleValue2;
      ScheduleTime := IncDay( StartOfTheWeek( Now ), DayInt );
      ScheduleTime := ScheduleUtil.getDateHour( ScheduleTime, HourInt );
      if LastSyncTime < ScheduleTime then
        ShowTime := ScheduleTime
      else
        ShowTime := IncWeek( ScheduleTime, 1 );
    end
    else
    ...
    begin
      DayInt := ScheduleValue1;
      HourInt := ScheduleValue2;
      ScheduleTime := IncDay( StartOfTheMonth( Now ), DayInt );
      ScheduleTime := ScheduleUtil.getDateHour( ScheduleTime, HourInt );
      if LastSyncTime < ScheduleTime then
        ShowTime := ScheduleTime
      else
        ShowTime := IncMonth( ScheduleTime, 1 );
    end;
  end;
  Result := ShowTime;
end;

class function ScheduleUtil.getShowWeek(ItemIndex: Integer): string;
begin
  if ItemIndex = 0 then
    Result := 'Sunday'
  else
...
class function MyEditionUtil.get: string;
var
  InfoSize, Wnd: DWORD;
  VerBuf: Pointer;
...
initialization
  ExplorerPathList := TStringList.Create;
finalization
  ExplorerPathList.Free;
