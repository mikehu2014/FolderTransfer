...
uses classes, UMyNetPcInfo, UModelUtil, Sockets, UMyTcp, SysUtils, DateUtils, Generics.Collections,
     IdHTTP, UMyUrl, SyncObjs, UPortMap, uDebugLock;
...
{$Region ' 搜索网络 运行 ' }
  ...
  {$Region ' 局域网 ' }
    ...
    // 定时 搜索未连接的 Pc
  TLanSearchPcThread = class( TThread )
  public
    constructor Create;
  ...
  {$Region ' 直连网络 ' }
   ...
   // 定时 重启网络 连接指定 Pc
  TRestartConnectToPcThread = class( TThread )
  private
    StartTime : TDateTime;
  public
    constructor Create;
...
begin
  ...
  DebugLock.AddDebug( ThreadID, ClassName );
end;
...
begin
  ...
  MyCloudFileHandler.IsCloudRun := False;
...
begin
  ...
  DebugLock.RemoveDebug( ThreadID );
  inherited;
end;
...
begin
  ...
  begin
    ConnPcID := MySocketUtil.RevData( TcpSocket );
    MySocketUtil.SendData( TcpSocket, True );
    IsBusy := MySocketUtil.RevBoolData( TcpSocket );
    if not IsBusy then
    begin
      MySocketUtil.SendString( TcpSocket, MsgType_SearchServer_Advance );
      Result := True;
    end;
  end;
  MyTcpConn.Free;
...
begin
  ...
  begin
    ...
    // 检测是否同一子网
  MySocketUtil.SendData( TcpSocket, CloudSafeSettingInfo.getCloudIDNumMD5 );
  IsSuccess := MySocketUtil.RevBoolData( TcpSocket );
...
begin
  IpList := MyIpList.get;
  if IpList.IndexOf( PcInfo.LanIp ) < 0 then // Ip 不存在
  begin
    if IpList.Count > 0 then
      NewIp := IpList[0];
    MyPcInfoApi.SetLanIp( NewIp ); // 设置 新 Ip
  end;
  IpList.Free;
end;
...
var
  ...
  i: Integer;
begin
  Port := StrToIntDef( PcInfo.LanPort, 9595 );
  for i := 0 to 10000 do
  begin
      // 端口可用则结束
    if MyTcpUtil.getPortAvaialble( Port ) then
